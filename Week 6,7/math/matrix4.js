...
Matrix4.prototype = {
  ...
  multiplyVector: function(v) {
    ...
    const inputVectorValues = [v.x, v.y, v.z, v.w];
    for (let i = 0; i < 4; i++) {
      let columnTotal = 0;
      for (let j = 0; j < 4; j++) {
        columnTotal += this.elements[i * 4 + j] * inputVectorValues[j];
      }
      switch (i) {
        case 0:
          result.x = columnTotal;
          break;
        case 1:
          result.y = columnTotal;
          break;
        case 2:
          result.z = columnTotal;
          break;
        case 3:
          result.w = columnTotal;
          break;
      }
    }
    return result;
  ...
  multiply: function(rightSideMatrix) {
    ...
    // todo - multiply 'this' * rightSideMatrixs

    let result = new Matrix4();
    for (let i = 0; i < 16; i++) {
      let columnTotal = 0;
      for (let j = 0; j < 4; j++) {
        columnTotal += this.elements[Math.floor(i / 4) * 4 + j] * rightSideMatrix.elements[j * 4 + i % 4];
      }
      result.elements[i] = columnTotal;
    }
    this.copy(result);
    return this;
  },
  ...
  makeScale: function(x, y, z) {
    ...
    this.makeIdentity();
    const scaleAmounts = [x, y, z];
    for (let i = 0; i < 11; i++) {
      if (i % 5 == 0) {
        this.elements[i] = scaleAmounts[Math.floor(i / 5)];
      }
    }
    return this;
  },
  ...
  makeRotationX: function(degrees) {
    ...
    rotationMatrix.elements = Float32Array.from([
      1, 0, 0, 0,
  ...
  makePerspective: function(fovy, aspect, near, far) {
    ...
    projectionMatrix.elements = Float32Array.from([
      near / r, 0, 0, 0,
      0, near / t, 0, 0,
    ...
    return this.multiply(projectionMatrix);
  },
  ...
  createTRSMatrix: function(translation, rotation, scale) {
    ...
    return new Matrix4().multiply(translation).multiply(rotation).multiply(scale);
  },
  ...
  createMoonMatrix: function(currentRotationAngle, offsetFromEarth, earthTransform) {
    ...
    // const earthTransformMatrix = new Matrix4.multiply(earthTransform);
    // const offsetFromEarthMatrix = new Matrix4().makeTranslation(offsetFromEarth);
    // const rotationMatrix = new Matrix4.makeRotationX(currentRotationAngle);

    // const TRSMatrix = createTRSMatrix()

    // moonMatrix.multiply(new Matrix4().makeRotationZ(currentRotationAngle)).multiply(new Matrix4().makeTranslation(offsetFromEarth).multiply(earthTransform));
    moonMatrix.multiply(new Matrix4().makeRotationZ(currentRotationAngle)).multiply(new Matrix4().makeTranslation(offsetFromEarth).multiply(earthTransform));
